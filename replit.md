...
- `scripts/cross-check-sources.ts`: Automated cross-checking tool that identifies all entity-attribute pairs and fetches missing data from Wikipedia, World Bank, and Wikidata. Features comprehensive deduplication checking by (entity, attribute, source_trust) tuple. Accessible via Admin page UI button with real-time statistics. Expected runtime: 2-5 minutes for full dataset depending on API response times.
- `scripts/fulfill-requested-facts.ts`: **NEW** - Processes requested_facts table, attempts to fetch data from Wikidata/World Bank for unsupported entity-attribute combinations, inserts into facts_evaluation if found, and removes fulfilled requests. Supports attribute-to-source mapping for intelligent source selection. Handles deduplication and cleanup automatically.
